...
public class PlayerListener implements Listener {
    ...
    public void onPlayerDropItem(PlayerDropItemEvent event) {
        ...
        if (!itemConfig.shouldDropItems()) {
            event.setCancelled(true);
            player.sendMessage(Text.createTextWithLang("arena.cannot-drop-items").build(player));
        }
    }
    ...
    public void onPlayerCommand(PlayerCommandPreprocessEvent event) {
        ...
        for (String blocked : commands) {
            if (command.startsWith(blocked.toLowerCase())) {
                event.setCancelled(true);
                player.sendMessage(Text.createTextWithLang("arena.cannot-use-command").build(player));
                break;
            }
        }
    }
    ...
    public void onPlayerTeleport(PlayerTeleportEvent event) {
        ...
        if (arena.getState() == ArenaState.ACTIVE) {
            event.setCancelled(true);
            player.sendMessage(Text.createTextWithLang("arena.cannot-teleport").build(player));
        }
    }
